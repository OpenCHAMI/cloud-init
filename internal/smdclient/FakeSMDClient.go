...
// FakeSMDClient is a simple in-memory SMD client used for demos and tests.
type FakeSMDClient struct {
	...
	rosettaMapping []SMDRosettaStone
}

// SMDRosettaStone maps a component ID to its network identity used by tests.
...
// NewFakeSMDClient builds a fake SMD client with a synthetic inventory.
func NewFakeSMDClient(clusterName string, count int) *FakeSMDClient {
	client := &FakeSMDClient{}
	client.clusterName = clusterName
	componentMap, rosetta, err := generateFakeComponents(count, "10.20.30.0/20")
	if err != nil {
		panic(err)
	}
	client.components = componentMap
	client.rosettaMapping = rosetta
...
// IDfromMAC returns the component ID for a given MAC.
func (f *FakeSMDClient) IDfromMAC(mac string) (string, error) {
	for _, c := range f.rosettaMapping {
		if c.BootMAC == mac {
...
// IDfromIP returns the component ID for a given IP or WG IP.
...
// IPfromID returns the boot IP for a given component ID.
...
// MACfromID returns the boot MAC for a given component ID.
...
// GroupMembership returns the synthetic group labels for a given component ID.
...
// ComponentInformation returns the base.Component for the given ID.
func (f *FakeSMDClient) ComponentInformation(id string) (base.Component, error) {
	...
	log.Debug().Msgf("FakeSMDClient: %d components from %s to %s", len(f.components), f.rosettaMapping[0].ComponentID, f.rosettaMapping[len(f.rosettaMapping)-1].ComponentID)
...
// Summary prints a brief summary of the synthetic inventory.
func (f *FakeSMDClient) Summary() {
	fmt.Printf("FakeSMDClient: %d components from %s to %s\n", len(f.components), f.rosettaMapping[0].ComponentID, f.rosettaMapping[len(f.rosettaMapping)-1].ComponentID)
...
func generateFakeComponents(numComponents int, cidr string) (map[string]base.Component, []SMDRosettaStone, error) {
	...
	for i := 1; i <= numComponents; i++ {
		...
		xname, _ = incrementXname(xname) // ignoring error because we know the format is correct
...
// PopulateNodes is a no-op for the fake client.
...
func (f *FakeSMDClient) AddNodeToInventory(node cistore.OpenCHAMIComponent) error {
	...
	f.rosettaMapping = append(f.rosettaMapping, SMDRosettaStone{
		ComponentID:   node.ID,
		BootMAC:       node.MAC,
...
// ListNodes returns all nodes in the fake inventory.
...
// UpdateNode updates a node's component data and network identity.
func (f *FakeSMDClient) UpdateNode(node cistore.OpenCHAMIComponent) error {
	...
	f.components[node.ID] = node.Component
	for i, c := range f.rosettaMapping {
		if c.ComponentID == node.ID {
			if node.MAC != "" {
				f.rosettaMapping[i].BootMAC = node.MAC
			}
			if node.IP != "" {
				f.rosettaMapping[i].BootIPAddress = node.IP
			}
			break
		}
	}
...
// AddWGIP sets the WireGuard IP for the given node ID.
func (f *FakeSMDClient) AddWGIP(id string, wgip string) error {
	for idx, c := range f.rosettaMapping {
		if c.ComponentID == id {
			c.WGIPAddress = wgip
			f.rosettaMapping[idx] = c
			return nil
		}
	}
...
// WGIPfromID returns the WireGuard IP for the given node ID.
